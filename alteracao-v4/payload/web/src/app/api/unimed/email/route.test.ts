...
describe("Unimed email API", () => {
  ...
  it("explains when the SMTP environment is not configured", async () => {
  ...
  it("explains when Gmail rejects the application password", async () => {
    mocks.sendUnimedExclusionEmail.mockRejectedValue(
      new Error("SMTP_AUTH_ERROR"),
    );
    const response = await POST(emailRequest(validInput));
    expect(response.status).toBe(503);
    await expect(response.json()).resolves.toMatchObject({
      error: { code: "SMTP_AUTH_ERROR" },
    });
  });

  it("explains when the SMTP server rejects a recipient", async () => {
    mocks.sendUnimedExclusionEmail.mockRejectedValue(
      new Error("SMTP_RECIPIENT_ERROR"),
    );
    const response = await POST(emailRequest(validInput));
    expect(response.status).toBe(502);
    await expect(response.json()).resolves.toMatchObject({
      error: { code: "SMTP_RECIPIENT_ERROR" },
    });
  });
  ...
  it("does not expose recipients, beneficiary data or service errors", async () => {
    mocks.sendUnimedExclusionEmail.mockRejectedValue(
      new Error(
        "CPF 12345678900 recipient private@example.com SMTP_PASSWORD secret",
      ),
    );
