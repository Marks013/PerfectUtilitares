...
describe("Unimed price history seed", () => {
  ...
  it("contains the historical table valid through July 2026", () => {
    ...
    expect(period.every((row) => row.validTo === "2026-07-31")).toBe(true);
    expect(sql).toContain(
      ":'tenant_id', 'FUNERAL', 'Aditivo funeral', 5.42, DATE '2024-07-01', DATE '2026-07-31'",
    );
  });
  ...
  it("defines automatic day-25 billing closure for both price periods", () => {
    expect(sql).toContain(
      ":'tenant_id', 'AUTOMATIC_DAY_25', 25, DATE '2024-07-01', DATE '2026-07-31'",
    );
    expect(sql).toContain(
