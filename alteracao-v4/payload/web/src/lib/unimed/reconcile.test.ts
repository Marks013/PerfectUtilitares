...
describe("Unimed source reconciliation", () => {
  ...
  it("links a dependent by the holder registration when the invoice omits the holder", () => {
    const holder = beneficiary(
      "H1",
      "Titular Exemplo",
      "HOLDER",
      "52998224725",
    );
    holder.registration = "4689";
    const dependent = beneficiary(
      "D1",
      "Dependente Novo",
      "DEPENDENT",
      "11144477735",
    );
    dependent.registration = "4689";

    const result = reconcileUnimedSources([holder, dependent], [], []);

    expect(
      result.beneficiaries.find((item) => item.sourceKey === "D1"),
    ).toMatchObject({ holderSourceKey: "H1" });
    expect(result.warnings.unmatchedDependents).toBe(0);
    expect(result.information.dependentsLinkedByRegistration).toBe(1);
  });

  it("reuses the immediately previous competency link when the current invoice has no holder reference", () => {
    const holder = beneficiary(
      "H-CURRENT",
      "Titular Exemplo",
      "HOLDER",
    ...
    const dependent = beneficiary(
      "D-CURRENT",
      "Martina Evangelista da Costa",
      "DEPENDENT",
      "11144477735",
    );
    dependent.registration = "D-2026";
    ...
    const result = reconcileUnimedSources([holder, dependent], [], [], [
      {
        dependent: {
          ...
          registration: "D-2025",
          cpf: "11144477735",
          fullName: "Martina Evangelista da Costa",
        },
        holder: {
          branchCode: "MATRIZ",
          registration: "4689",
          cpf: "52998224725",
          fullName: "Titular Exemplo",
        },
      },
    ]);

    expect(
      result.beneficiaries.find((item) => item.sourceKey === "D-CURRENT"),
    ).toMatchObject({ holderSourceKey: "H-CURRENT" });
    expect(result.warnings.unmatchedDependents).toBe(0);
    expect(
      result.information.dependentsLinkedFromPreviousCompetency,
    ).toBe(1);
  });
