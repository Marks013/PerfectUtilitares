...
export async function POST(request: Request) {
  ...
  try {
    ...
    const selectedDependents = parsed.data.dependentIds.map((dependentId) =>
      dependentById.get(dependentId)!,
    );
    ...
    const holderPricing = pricingFor(beneficiary);
    const dependentPricing = selectedDependents.map((dependent) => ({
      dependent,
    ...
    if (
      ...
      return jsonError(
        ...
        "Não há preço oficial configurado para um dos planos na data informada.",
      );
    }
    if (
      (beneficiary.hasAddon ||
        selectedDependents.some((dependent) => dependent.hasAddon)) &&
      configuration.addonPrices.length !== 1
    ) {
      return jsonError(
        422,
        "UNIMED_ADDON_NOT_CONFIGURED",
        "Configure um único valor vigente para o Acessório Funeral.",
      );
    }

    const addonAmount = configuration.addonPrices[0]?.amount.toNumber() ?? 0;
    const officialInput = {
      ...
      holder: {
        ...
        addonAmount: beneficiary.hasAddon ? addonAmount : 0,
      },
