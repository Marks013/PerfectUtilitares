...
function readNumber(values: ArrayLike<number>, index: number, context: string) {
  const value = values[index];
  if (value === undefined) throw new Error(`Valor numérico ausente durante ${context}.`);
  return value;
}
function median(values: number[]) {
  ...
  const upper = readNumber(sorted, middle, "o cálculo da mediana");
...
export function classifyRenderedColors(pixels: Uint8ClampedArray): CompressionColorMode {
  let contentPixels = 0;
  let coloredPixels = 0;
  ...
  for (let pixel = 0; pixel < pixelCount; pixel += step) {
    ...
    if (channelDelta >= 10) coloredPixels += 1;
    else if (lightness > 28 && lightness < 228) intermediateGrayPixels += 1;
  }
  if (!contentPixels) return "MONOCHROME";
  if (coloredPixels >= 40 && coloredPixels / contentPixels >= 0.003) return "COLOR";
  if (intermediateGrayPixels / contentPixels >= 0.02) return "GRAYSCALE";
  return "MONOCHROME";
}
...
export async function analyzePdfForCompression(
  ...
  const loadingTask = pdfjs.getDocument({
    ...pdfJsClientDocumentOptions(new Uint8Array(await file.arrayBuffer())),
    stopAtErrors: false,
  });
  ...
  try {
    for (const pageNumber of sampledPageNumbers) {
      ...
      const renderScale = Math.min(1, 640 / Math.max(viewport.width, viewport.height));
      const renderViewport = page.getViewport({ scale: renderScale });
      const canvas = window.document.createElement("canvas");
      canvas.width = Math.max(1, Math.ceil(renderViewport.width));
      canvas.height = Math.max(1, Math.ceil(renderViewport.height));
      const context = canvas.getContext("2d", { willReadFrequently: true });
      if (!context) throw new Error("O navegador não permitiu analisar as páginas.");
      ...
      for (let operationIndex = 0; operationIndex < operatorList.fnArray.length; operationIndex += 1) {
        ...
        if (operation === pdfjs.OPS.paintImageXObject) {
          ...
          if (typeof width === "number" && width > 0 && typeof height === "number" && height > 0) {
            ...
            try { dimensions = asImageDimensions(page.objs.get(objectId)); } catch { dimensions = null; }
          }
        ...
        const coverage = Math.min(1, (widthPoints * heightPoints) / Math.max(1, viewport.width * viewport.height));
        pageMaxImageCoverage = Math.max(pageMaxImageCoverage, coverage);
        ...
        if (effectiveDpi >= 36 && effectiveDpi <= 2_400) effectiveDpis.push(effectiveDpi);
      }
      if (pageMaxImageCoverage >= 0.82) fullPageImagePages += 1;
      page.cleanup();
    }
  } finally {
  ...
  const colorMode = colorModes.includes("COLOR")
    ? "COLOR"
    : colorModes.includes("GRAYSCALE") ? "GRAYSCALE" : "MONOCHROME";
  return {
    fileKey,
    fileName: file.name,
    pageCount,
