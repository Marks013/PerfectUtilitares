...
describe("Unimed calculation engine", () => {
  ...
  it("adds one full installment after the day-25 cutoff", () => {
    ...
    expect(result.currentCompetency).toBe("2026-08");
    expect(result.nextCompetency).toBe("2026-09");
    expect(result.nextCompetencyDays).toBe(30);
    expect(result.totalRefundDays).toBe(31);
  ...
  it("uses independent price tables for the proportional month and the next full competency", () => {
    const result = calculateUnimed({
      reasonCode: 3,
    ...
    expect(result.currentCompetencyRefund).toBe("19.35");
    expect(result.nextCompetencyRefund).toBe("120.00");
    expect(result.nextCompetencyInvoiceTotal).toBe("120.00");
    expect(result.employeeCurrentRefund).toBe("11.61");
    expect(result.employeeNextRefund).toBe("70.00");
    expect(result.employeeFullRefund).toBe("81.61");
    expect(result.companyFullRefund).toBe("57.74");
    expect(result.invoiceRefund).toBe("139.35");
    expect(result.refundDays).toBe(6);
    expect(result.nextCompetencyDays).toBe(30);
    expect(result.totalRefundDays).toBe(36);
  });
